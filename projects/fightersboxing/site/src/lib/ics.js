// THE SUBSCRIBE FEEDS
//
// Builds an .ics from src/data/schedule.js at build time. Each session
// becomes one weekly repeating event, so a member subscribes once and the
// gym's week sits in their phone calendar for good. Change a time in the
// dataset, redeploy, and every subscriber's calendar updates on its next
// refresh (12 hours, per REFRESH-INTERVAL below).
//
// Two pages call this, one per audience: /schedule.ics for the adult
// classes and /youth-schedule.ics for the kids. Same dataset, so they can
...
// having to guess.
import { pick, days } from '../data/schedule.js';
...
/**
 * @param {{ audience: string, name: string }} feed
 * @returns {Response} the calendar file
 */
export function buildIcs(feed) {
  const icsDay = Object.fromEntries(days.map((d) => [d.key, d.ics]));
  const feedSessions = pick({ audience: feed.audience });

  const events = feedSessions.map((s, i) => {
    const date = ANCHOR[s.day];
    return [
      'BEGIN:VEVENT',
      line(
        'UID',
        `${feed.audience}-${s.day}-${s.start.replace(':', '')}-${slug(s.name)}-${i}@fightersnashville.com`
      ),
      line('DTSTAMP', STAMP),
  ...
  return new Response(body, {
    headers: {
      ...
      'Content-Disposition': `inline; filename="fighters-boxing-${feed.audience}-schedule.ics"`,
    },
  });
}
