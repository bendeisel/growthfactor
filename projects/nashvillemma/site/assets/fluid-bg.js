// Ambient fluid gradient, ported from the Fighters Boxing build
// (projects/fightersboxing/site/src/scripts/fluid-bg.js), which was itself a
// vanilla port of the PixelLiquidBg React component Ben supplied.
//
// Same simulation, retuned for Nashville MMA: Fighters runs faded red on a
// white ground, this runs the kernel golds on near-black.
//
// Motion is a requestAnimationFrame loop, never a CSS animation, per
// .claude/skills/bmfg-gym-sites/references/motion.md.
//
// `three` resolves through the importmap in the page shell to the copy
// vendored at assets/three.module.min.js, so there is no CDN dependency and
// no build step.
import {
  WebGLRenderer,
  WebGLRenderTarget,
...
export function mountFluidBg(container, opts = {}) {
  ...
  const outputUniforms = {
    ...
    uBayer: { value: bayer },
    // near-black ground, alpha 0 so the page shows through at rest
    bgColor: { value: new Vector4(0.02, 0.02, 0.02, 0) },
    uTime: { value: 0 },
